...
public class Table{
	...
	public static String[] getColumnNames(String table){ //tables=davisbase_tables
		String[] columnNames = new String[0];
		try{
			RandomAccessFile file = new RandomAccessFile("data/davisbase_columns.tbl", "rw");
			Payload payload = new Payload();

			// use davisbase_columns.tbl to fetch all columns of a table
			String[] columns = {"rowid", "table_name", "column_name", "data_type", "ordinal_position", "is_nullable"};
			String[] cmp = {"table_name","=",table};

			filter(file, cmp, columns, payload);

			HashMap<Integer, String[]> content = payload.content;
			ArrayList<String> records = new ArrayList<String>();
			for(String[] record : content.values()){
				records.add(record[2]);
			}
			int size=records.size();
			columnNames = records.toArray(new String[size]);
			file.close();
	...
	public static String[] getDataType(String table){
		...
		try{
			...
			filter(file, cmp, columnName, payload);
			...
			int size=records.size();
			dataType = records.toArray(new String[size]);
			file.close();
	...
	public static void filter(RandomAccessFile file, String[] cmp, String[] columnNames, Payload payload){
		try{
			...
			for(int page = 1; page <= numOfPages; page++){
				...
				if(pageType == 0x0D)
				{
					...
					for(int i=0; i < numOfCells; i++){
						...
						String[] value = retrieveValues(file, loc);
						int rowid = Integer.parseInt(value[0]);

						boolean check = cmpCheck(value, rowid, cmp, columnNames);

						if(check)
							payload.add_record(rowid, value);
					}
				}
				else
					continue;
			...
			payload.columnName = columnNames;
			payload.format = new int[columnNames.length];
			DavisbaseUtility.print(payload.toString());
	...
	public static int getPayloadSize(String table, String[] vals, byte[] stc){
	...
	public static void filter(RandomAccessFile file, String[] cmp, String[] columnName, String[] type, Payload payload){
		try{
			...
			payload.format = new int[columnName.length];
