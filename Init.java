...
public class Init {
	...
	public static void initialize() {
		...
		try {
			...
			RandomAccessFile tablesCatalog = new RandomAccessFile(DavisbaseUtility.db_tables, "rw");
		...
		try {
			RandomAccessFile columnsCatalog = new RandomAccessFile(DavisbaseUtility.db_columns, "rw");
			columnsCatalog.setLength(pageSize);
			columnsCatalog.seek(0);
			columnsCatalog.writeByte(0x0D); // leaf table of b-tree
			columnsCatalog.writeByte(0x08); // An array of 2-byte integers that indicate the page offset location of
											// each data cell.The array size is 2n, where n is the number of cells on
											// the page. The array is maintained in key-sorted order—i.e. rowid order
											// for a table file and index order for an index file.
